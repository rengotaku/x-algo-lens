...
解説ページの図（候補ソースの上限バーチャート等）は台帳から組み立てる。
そのため図が使う値は `role` の散文ではなく、構造化フィールドとして持たせる。

| フィールド | 対象 | 値 |
|---|---|---|
| `availability` | source | `default_on` / `default_off` / `conditional` |
| `limit_default` | source | 既定の取得上限（整数、無ければ `null`） |

値の裏付けは同じエントリの `evidence` にある。
構造化されていないと `make site` が落ちる（図を推測で描かないため）。
